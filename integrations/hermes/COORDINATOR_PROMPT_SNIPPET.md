...
2. After editing any skill or heal path, the human (or CI) must run `python data/seed.py` then `python harness.py` (five experiments + `integrations/validate.py` gate).  
...
4. Prefer **small diffs** with a short rationale tied to a failure class (OOM, crash loop, probe, latency, K8s signal class). Experiment 5 stresses **noisy** log lines — avoid breaking zero–false-positive healthy slots in `near_real_stream.json`.
